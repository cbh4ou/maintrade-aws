...
const Miso = () => {
  ...
  const auctions = useAuctions(queryToAuctionStatus[query?.status as string] ?? AuctionStatus.FINISHED)

  const tabs = [
    { key: AuctionStatus.FINISHED, value: i18n._(t`All Strategies`), link: 'finished' },
    { key: AuctionStatus.LIVE, value: i18n._(t`Dual Momentum`), link: 'live' },
    { key: AuctionStatus.UPCOMING, value: i18n._(t`DCA (3Commas)`), link: 'upcoming' },
  ]
        ...
        <div className="flex justify-between lg:flex-row flex-col gap-8">
          <div className="flex flex-col">
            ...
            <Typography weight={700}>
              {i18n._(t`Discover and apply different strategies to your portfolios.`)}
            </Typography>
          </div>
      ...
      <MisoBody>
        <section className="flex flex-col gap-10">
          <div className="flex flex-col">
            ...
            >
              {tabs.map((tab) => (
                <Link href={`/explore?status=${tab.link}`} key={tab.key} passHref={true}>
